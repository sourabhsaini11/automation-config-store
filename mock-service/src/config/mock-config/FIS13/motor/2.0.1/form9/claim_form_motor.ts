...
export class MockClaimFormMotorClass extends MockAction {
	name(): string {
		return "claim_form_motor";
	}
	get description(): string {
		return "Mock for claim_form_motor";
	}
	...
	async validate(
		...
		const formLink = sessionData["claim_form_motor"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			claim_form_motor: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { claim_form_motor: "claim_form_motor" } };
	}
	get defaultData(): any {
		return {};
	}
