...
export class MockPaymentFailureFormClass extends MockAction {
	name(): string {
		return "payment_failure_form";
	}
	get description(): string {
		return "Mock for payment_failure_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["payment_failure_form"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			payment_failure_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { payment_failure_form: "payment_failure_form" } };
	}
	get defaultData(): any {
		return {};
	}
