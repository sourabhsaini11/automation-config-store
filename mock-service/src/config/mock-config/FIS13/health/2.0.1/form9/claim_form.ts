...
export class MockClaimFormClass extends MockAction {
	name(): string {
		return "claim_form";
	}
	get description(): string {
		return "Mock for claim_form";
	}
	...
	async validate(
		...
		const formLink = sessionData["claim_form"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			claim_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { claim_form: "claim_form" } };
	}
	get defaultData(): any {
		return {};
	}
