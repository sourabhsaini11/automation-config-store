...
export class MockOnStatusCompleteBus201Class extends MockAction {
  ...
  name(): string {
    return "on_status_complete_BUS_201";
  }
...
export class MockUnsoliciatedOnStatusCompleteBus201Class extends MockAction {
  get saveData(): saveType {
    return yaml.load(
      readFileSync(path.resolve(__dirname, "../save-data.yaml"), "utf8")
    ) as saveType;
...
async validate(
  ...
  const authStop = fulfillment?.stops?.find(
    (stop: any) => stop?.authorization
  );

  return authStop?.authorization?.status !== "CLAIMED";
  });
  ...
  return {
    valid: false,
    message: `Invalid authorization status at fulfillment index ${invalidIndex}. Expected "CLAIMED", got ${authStop?.authorization?.status}`,
  };
  }
