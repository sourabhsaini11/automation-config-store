...
export class MockPaymentFormClass extends MockAction {
	...
	async validate(
		...
		const formLink = sessionData["payment_form"];
		if (!formLink) {
	...
	override async __forceSaveData(
		...
		return {
			...sessionData,
			payment_form: resolveFormActions(formLink, formData),
		};
	}
	...
	get saveData(): saveType {
		return { "save-data": { payment_form: "payment_form" } };
	}
	get defaultData(): any {
		return {};
	}
