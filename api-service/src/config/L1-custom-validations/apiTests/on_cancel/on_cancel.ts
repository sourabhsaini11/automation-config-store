    ...
    try {
      if (sumQuoteBreakUp(on_cancel.quote) && flow != "6") {
        ...
        const priceAtConfirm = priceAtConfirmRaw
          ...
          flow === "5"
            ? _.filter(on_cancel.fulfillments, (f) => ["RTO", "Cancel"].includes(f.type))
            : _.filter(on_cancel.fulfillments, { type: "Cancel" });
        
        if (flow === "5") {
          const rtoFulfillments = _.filter(on_cancel.fulfillments, { type: "RTO" });
          const cancelOnlyFulfillments = _.filter(on_cancel.fulfillments, { type: "Cancel" });
          cancelFulfillments = _.uniqBy([...rtoFulfillments, ...cancelOnlyFulfillments], 'id');  // merge both, avoid duplicates by `id`
        }
