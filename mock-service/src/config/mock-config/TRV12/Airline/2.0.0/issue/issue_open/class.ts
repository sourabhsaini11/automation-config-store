...
export class MockIssueOpenMetro_210_Class extends MockAction {
...
export class MockIssueEscalateMetro_210_Class extends MockAction {
  get saveData(): saveType {
    return yaml.load(
      readFileSync(path.resolve(__dirname, "../save-data.yaml"), "utf8")
    ) as saveType;
  ...
  name(): string {
    return "issue_escalate";
  }
  get description(): string {
    return "Mock for issue_escalate";
  }
  generator(existingPayload: any, sessionData: SessionData): Promise<any> {
    return issueStatusGenerator(
      existingPayload,
