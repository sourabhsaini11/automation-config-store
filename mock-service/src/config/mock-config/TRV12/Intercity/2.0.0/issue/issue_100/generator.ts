...
export const issueStatusGenerator_100 = async (
  ...
  if (sessionData.fulfillments) {
    fulfillment = sessionData?.fulfillments?.flat()?.[0]?.id;
  }
  const item = sessionData?.items?.flat()?.[0]?.id
  const itemCount = sessionData?.items?.flat()?.[0]?.quantity?.selected?.count
  ...
  switch (sessionData.igm_action) {
    case "issue_open":
      ...
      existingPayload.message.issue.complainant_info = {
        ...
        contact: {
          phone: sessionData?.billing?.phone ?? "+91-9897867564",
